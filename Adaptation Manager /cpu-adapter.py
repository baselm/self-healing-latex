...
def run_adaptation_strategy(attempt, cpu):
  ...
  if attempt > 1 and cpu > 70 and Replicas <=20 :
    ...
    print "Replicas <= 20 Adaptation started- ", "attempt: ",attempt, 'Replicas: ' ,Replicas, 'cpu' ,cpu
    time.sleep(300)
  elif attempt < 1 and cpu > 70 and Replicas <= 20:
     
    Replicas += 2 
    cmd = "docker service scale " + "web" + "=" + str(Replicas)
    print "First attempt < 1 and cpu > 70 Adaptation started ", "attempt: ",attempt, 'Replicas: ' ,Replicas, 'cpu' ,cpu

    res= subprocess.check_output(cmd, shell=True)
    time.sleep(300)
  elif attempt > 1 and cpu < 40 and Replicas <= 20 and Replicas >= 1:
    print "attempt > 1 and cpu < 40 and Replicas <= 20 Adaptation started- ", "attempt: ",attempt, 'Replicas: ' ,Replicas, 'cpu' ,cpu
    Replicas -= 1 
    if Replicas >=1:
      cmd = "docker service scale " + "web" + "=" + str(Replicas)
      print "attempt > 1 and cpu < 40 and Replicas <= 20 Adaptation started- ", "attempt: ",attempt, 'Replicas: ' ,Replicas, 'cpu' ,cpu
    ...
    print "attempt < 1 and cpu < 40 and Replicas >= 1", "attempt: ",attempt, 'Replicas: ' ,Replicas, 'cpu' ,cpu
    Replicas -= 1
    if Replicas >=1:
      cmd = "docker service scale " + "web" + "=" + str(Replicas)
      print "attempt < 1 and cpu < 40 and Replicas >= 1", "attempt: ",attempt, 'Replicas: ' ,Replicas, 'cpu' ,cpu 
